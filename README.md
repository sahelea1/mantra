...
How it runs: one `claude -p --input-format stream-json --output-format stream-json --verbose --dangerously-skip-permissions --session-id <uuid> --model <m> [--effort e] --autocompact <ctx>` process per agent, prompts on stdin, events translated into the same shapes the UI already renders (streaming text, tool calls, compaction, token usage). Mid-turn messages arrive at the next tool boundary (that's `ctrl+f`), `x` sends a real interrupt, `/compact` compacts the session, a crashed process is restarted with `--resume`, and Mantra's own tools (`mantra_submit_plan`, `mantra_spawn`, …) reach the agent through a tiny MCP server (`mantra mcp-bridge`) that Mantra starts for it. Running as root, Mantra sets `IS_SANDBOX=1` on the child itself (Claude Code's rule for `--dangerously-skip-permissions`); `mantra doctor` notes it. `mantra doctor` also checks `claude --version` (5 s timeout) and each Claude provider's auth. The built-in pattern `mantra-default-claude` (`/pattern`) runs planner and orchestrator on `fable51` and workers on `sonnet5` with the gates on Codex — a mixed team out of the box.
...
Then press **`D`** on that provider (or `D` on the models table to discover everything, including Codex's own catalog). On a `claude-code` provider `D` always lists the six defaults and, when a `base_url` is set, whatever the gateway's `/v1/models` returns. Mantra fetches `GET {base_url}/models` with your key and opens a picker (type to filter, `space` select, `tab` all/none, `⏎` add & save):
