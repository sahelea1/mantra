...
/// Claude Code providers only ever store the key's *env var name* so far (`ProviderEntry.env_key`,
/// same as Codex custom providers) — `resolve_key()` (WP5, landing separately) will replace this
/// with a helper that also checks a pasted `api_key` field; until then this is the only source.
fn claude_key(p: &crate::config::ProviderEntry) -> Option<String> {
    let name = p.env_key.trim();
    if name.is_empty() {
        return None;
    }
...
pub fn spawn_agent(hub: &mut Hub, agents: &mut BTreeMap<AgentId, Agent>, reg: &Registry, o: AgentOpts) -> AgentId {
    ...
    let (extra, envs, claude) = if backend == crate::hub::Backend::ClaudeCode {
        ...
        let mut envs = vec![];
        if auth == "api_key" {
            if let Some(key) = provider.and_then(claude_key) {
